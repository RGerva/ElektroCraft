...
public class ModUtils {
  ...
  public static class ModResistorUtil {
    public static int calculateResistance(
        ModItemProperties.ResistorColorCode first,
        ModItemProperties.ResistorColorCode second,
        ModItemProperties.ResistorColorCode multiplier) {
    ...
    private static final String[] RESISTANCE_PREFIXES =
        new String[] {"Ω", "kΩ", "MΩ", "GΩ", "TΩ", "PΩ"};
  ...
  public static class ModVoltageUtil {
    private static final String[] VOLTAGE_PREFIXES =
        new String[] {"μV", "mV", "V", "kV", "MV", "GV", "TV", "PV"};

    private static final double[] THRESHOLDS =
        new double[] {1e-6, 1e-3, 1, 1e3, 1e6, 1e9, 1e12, 1e15};
  ...
  public static class ModCurrentUtil {
    private static final String[] CURRENT_PREFIXES =
        new String[] {"nA", "μA", "mA", "A", "kA", "MA", "GA", "TA", "PA"};

    private static final double[] THRESHOLDS =
        new double[] {1e-9, 1e-6, 1e-3, 1, 1e3, 1e6, 1e9, 1e12, 1e15};
  ...
  public static class ModCapacitanceUtil {
    private static final String[] CAPACITANCE_PREFIXES =
        new String[] {"pF", "nF", "μF", "mF", "F", "kF", "MF", "GF"};

    private static final double[] THRESHOLDS =
        new double[] {1e-12, 1e-9, 1e-6, 1e-3, 1, 1e3, 1e6, 1e9};
    ...
    public static final Map<Item, Double> DIELECTRIC_CONSTANTS =
        Map.ofEntries(
  ...
  public static class ModRecipeUtil {
    public static <C extends RecipeInput, T extends Recipe<C>>
        Collection<RecipeHolder<T>> getAllRecipesFor(ServerLevel level, RecipeType<T> recipeType) {
    ...
    public static <C extends RecipeInput, T extends Recipe<C>> boolean isIngredientOfAny(
        ServerLevel level, RecipeType<T> recipeType, ItemStack itemStack) {
    ...
    public static <C extends RecipeInput, T extends Recipe<C>> List<Ingredient> getIngredientsOf(
        ServerLevel level, RecipeType<T> recipeType) {
      Collection<RecipeHolder<T>> recipes = getAllRecipesFor(level, recipeType);
  ...
  public static final class InventoryUtils {
    public static boolean canInsertItemIntoSlot(
        Container inventory, int slot, ItemStack itemStack) {
      ...
      if (ModCapacitanceUtil.isCapacitor(inventory.getItem(0))) {
        for (int i = 0; i < inventory.getContainerSize(); i++) {
          if (inventory.getItem(i).is(ModTags.Items.DIELECTRIC_CONSTANTS)) {
            double capacitance =
                ModUtils.ModCapacitanceUtil.computeCapacitance(inventory.getItem(i));
        ...
        if (ItemStack.isSameItem(inventoryItemStack, itemStack)) {
          ...
          if (Objects.equals(currentCap, newCap)) {
            return inventoryItemStack.getMaxStackSize()
                >= inventoryItemStack.getCount() + itemStack.getCount();
          }
        }

      } else {
        return inventoryItemStack.isEmpty()
            || (ItemStack.isSameItemSameComponents(inventoryItemStack, itemStack)
                && inventoryItemStack.getMaxStackSize()
                    >= inventoryItemStack.getCount() + itemStack.getCount());
      }
      return false;
    }
    ...
    public static int getRedstoneSignalFromItemStackHandler(ItemStackHandler itemHandler) {
      ...
      for (int i = 0; i < size; i++) {
        ...
        if (!item.isEmpty()) {
          fullnessPercentSum +=
              (float) item.getCount()
                  / Math.min(item.getMaxStackSize(), itemHandler.getSlotLimit(i));
          isEmptyFlag = false;
        }
      }
