...
public class ModItemTagProvider extends ItemTagsProvider {
  ...
  protected void addTags(HolderLookup.@NotNull Provider provider) {
    this.tag(ModTags.Items.WIRE_ITEMS).add(ModItems.TIN_SOLDER_WIRE.get());

    this.tag(ModTags.Items.RESISTORS)
        .add(ModItems.BLANK_RESISTOR.get())
        .add(ModItems.RESISTOR.get());

    this.tag(ModTags.Items.VOLTAGE).add(ModItems.DIODE.get());
    ...
    for (Item item : ModUtils.ModCapacitanceUtil.DIELECTRIC_CONSTANTS.keySet()) {
      this.tag(ModTags.Items.DIELECTRIC_CONSTANTS).add(item);
    }
