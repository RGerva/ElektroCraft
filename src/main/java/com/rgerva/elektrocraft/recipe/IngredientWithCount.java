/**
 * Record: IngredientWithCount Immutable data structure for simplified object representation.
 *
...
public record IngredientWithCount(Ingredient input, int count) {
  ...
  public static final Codec<IngredientWithCount> CODEC =
  ...
  public static final StreamCodec<RegistryFriendlyByteBuf, IngredientWithCount> STREAM_CODEC =
      new StreamCodec<>() {
        ...
        public IngredientWithCount decode(@NotNull RegistryFriendlyByteBuf buffer) {
          ...
          if (count <= 0) throw new DecoderException("Empty IngredientWithCount not allowed");
        ...
        @Override
        public void encode(
            @NotNull RegistryFriendlyByteBuf buffer, IngredientWithCount ingredient) {
